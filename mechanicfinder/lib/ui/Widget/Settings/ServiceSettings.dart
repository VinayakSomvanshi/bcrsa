...
class _ServiceSettingsState extends State<ServiceSettings> {
                      ...
                      child: ListTile(
                        ...
                        subtitle: Row(
                          ...
                          children: <Widget>[
                            ...
                            Text(
                              () {
                                return "Rs.${double.parse(_user['services'][index]['price']) - ((double.parse(_user['services'][index]['price']) * double.parse(_user['services'][index]['off'])) / 100)}";
                              }(),
                              style: GoogleFonts.lato(
          ...
          return StatefulBuilder(
            builder: (context, setstate) {
              return AlertDialog(
                ...
                title: Text(
                  "Add new Service - \n Rs.${double.parse(price) - ((double.parse(price) * double.parse(off)) / 100)}",
                  style: GoogleFonts.lato(color: grey),
                  textAlign: TextAlign.center,
                ),
                content: Container(
                              ...
                              onPressed: () {
                                if (_formkey.currentState!.validate()) {
                                  ...
                                  if (update) {
                                    ...
                                    _temp[updateindex]['price'] =
                                        double.parse(price).toStringAsFixed(2);
                                    _temp[updateindex]['off'] =
                                        double.parse(off).toStringAsFixed(2);
                                  } else {
                                    _temp.add({
                                      "name": name,
  ...
  Future<bool?> _removeservicemap(index) async {
    ...
    FirebaseFirestore.instance
        .collection("mechanic")
        .doc(_user.id)
        .update({"services": temp}).then((value) {
      FirebaseFirestore.instance
          .collection("mechanic")
          .doc(Home.userdata.id)
          .get()
