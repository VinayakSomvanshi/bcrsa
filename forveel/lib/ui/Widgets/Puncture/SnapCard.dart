...
// import 'package:flutter_advanced_networkimage/provider.dart';R
...
class _SnapcardState extends State<Snapcard> {
              ...
              children: <Widget>[
                Container(
                  ...
                  decoration: BoxDecoration(
                      ...
                      //   // ),
                      //    image: CachedNetworkImageProvider(widget.carddata['photo']),
                      //   fit: BoxFit.cover,
                      // )
                      ),
                ),
                ...
                (widget.index != widget.data.next && widget.data.progress > 60)
                    ? Container(
                        alignment: Alignment.center,
                        decoration: BoxDecoration(
                            borderRadius: BorderRadius.circular(15),
                            color: Colors.black
                                .withOpacity(widget.data.progress / 150)),
                      )
                    : Container()
              ],
            ),
                    ...
                    Column(
                      ...
                      children: <Widget>[
                        ...
                        Text(
                          "Direction",
                          style: GoogleFonts.lato(
                              color: grey, letterSpacing: 2, fontSize: 10),
                        )
                      ],
                    )
                  ],
                    ...
                    Card(
                      child: Container(
                        child: IconButton(
                          ...
                          onPressed: () async {
                            String url =
                                "whatsapp://send?phone=+91 ${widget.carddata['phone']}";
                            if (await canLaunch(url)) {
                              await launch(url);
                            } else {
                              throw 'Could not launch $url';
  ...
  void _launchMapsUrl() async {
    ...
    final url =
        'https://www.google.com/maps/dir/?api=1&origin=${widget.position.latitude},${widget.position.longitude}&destination=${widget.carddata["lat"]},${widget.carddata["long"]}&travelmode=driving';
    if (await canLaunch(url)) {
      await launch(url);
    } else {
      throw 'Could not launch $url';
