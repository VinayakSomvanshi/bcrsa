...
class MechanicUpdatePage extends StatefulWidget {
  ...
  String uservehicle;
  MechanicUpdatePage(
      this.mid, this.status, this.rid, this.issue, this.uservehicle);
  @override
...
class _MechanicUpdatePageState extends State<MechanicUpdatePage> {
    ...
    return Scaffold(
      ...
      body: NestedScrollView(
        ...
        body: Container(
          child: (load)
              ? Container(
                  decoration: BoxDecoration(color: LightColor.black),
                  child: ClipRRect(
                    child: BackdropFilter(
                      ...
                      child: ListView(
                        children: <Widget>[
                          Center(
                              child: VtypesShow(
                                  _mechanic['vehicle_type'],
                                  _mechanic['Two wheeler'],
                                  _mechanic['Four wheeler'])),
                          SizedBox(
                            height: 10,
                          ),
                                                ...
                                                Card(
                                                  child: Container(
                                                    child: IconButton(
                                                      ...
                                                      onPressed: () async {
                                                        String url =
                                                            "whatsapp://send?phone=+91 ${_mechanic['phone']}";
                                                        if (await canLaunch(
                                                            url)) {
                                                          await launch(url);
                                                        } else {
                          ...
                          ListTile(
                            ...
                            title: Text(
                              _mechanic['pickup']
                                  ? "Provides pickup from your location"
                                  : "Does not picks ups your vehicle from location",
                          ...
                          ExpansionTile(
                            ...
                            children: List.generate(
                                _mechanic['services'].length, (index) {
                              return ListTile(
                                onLongPress: () {},
                                title: Text(
                                  _mechanic['services'][index]['name'],
                                  style: GoogleFonts.lato(
                                ...
                                subtitle: Row(
                                  ...
                                  children: <Widget>[
                                    Text(
                                      "Rs. ${_mechanic['services'][index]['price']}",
                                      style: GoogleFonts.lato(
                                          color: grey,
                                          decoration:
                                    ...
                                    Text(
                                      () {
                                        return "Rs.${double.parse(_mechanic['services'][index]['price']) - ((double.parse(_mechanic['services'][index]['price']) * double.parse(_mechanic['services'][index]['off'])) / 100)}";
                                      }(),
                                      style: GoogleFonts.lato(
                                          color: green,
                                ...
                                trailing: Text(
                                  "${_mechanic['services'][index]['off']}% OFF",
                            ...
                            child: Row(
                              ...
                              children: <Widget>[
                                ...
                                Column(
                                  children: <Widget>[
                                    IconButton(
                                      icon: Icon(Icons.directions, color: pink),
                                      color: pink,
                          ...
                          Container(
                            margin: EdgeInsets.all(20),
                            child: (status == "pending" || status == "accepted")
                                ? ElevatedButton(
                                    style: ElevatedButton.styleFrom(
                                      primary: grey,
                                      shape: RoundedRectangleBorder(
  ...
  _refreshavalability() async {
    ...
    subscriptiontwo = FirebaseFirestore.instance
        .collection('service')
        .doc(widget.rid)
        .snapshots()
        .listen((event) {
  ...
  _fill_distance() async {
    double distanceInMeters = await Geolocator.distanceBetween(
        _position.latitude,
        _position.longitude,
        double.parse(_mechanic['lat'].toString()),
        double.parse(_mechanic['long'].toString()));
    setState(() {
  ...
  void _launchMapsUrl() async {
    ...
    final url =
        'https://www.google.com/maps/dir/?api=1&origin=${_position.latitude},${_position.longitude}&destination=${_mechanic['lat']},${_mechanic['long']}&travelmode=driving';
  ...
  _cancelit(text) async {
    if (await IsConnectedtoInternet()) {
      ShowInternetDialog(context);
      return;
    }
    LoaderDialog(context, false);
    FirebaseFirestore.instance.collection("service").doc(widget.rid).update({
      ...
      Fluttertoast.showToast(
          ...
          gravity: ToastGravity.CENTER,
          toastLength: Toast.LENGTH_SHORT);
      Navigator.popUntil(context as BuildContext, ModalRoute.withName('/'));
    });
