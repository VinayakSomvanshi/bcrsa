...
class _HomeState extends State<Home> {
  ...
  static const LatLng _center =
      const LatLng(19.26806793449255, 72.96724717955107);
...
/*     String city = await _GetCurrentCity();
    ...
    FirebaseFirestore.instance
        ...
        .where("onlypuncture", isEqualTo: false)
        .get()
        .then((value) {
      _mechanics = value.docs;
      _mechanicsstore = value.docs;
      filters.add(CustomPopupMenu(
          icon: Icons.remove,
          onclick: () {
            filtertitle = "Filter";
      ...
      if (_mechanics.isNotEmpty) {
        ...
        Fluttertoast.showToast(
            msg: "We don't have mechanics near you!",
              ...
              child: Row(
                  ...
                  children: <Widget>[
                    Container(
                        ...
                        height: 50,
                        child: ClipOval(
                            child: (_mechanics[pinindex]["photo"].isEmpty ||
                                    _mechanics[pinindex]["photo"] == null)
                      ...
                      child: GestureDetector(
                          ...
                          child: Column(
                            ...
                            children: <Widget>[
                              Text(
                                ...
                                style: GoogleFonts.lato(color: myyellow),
                        ...
                        onTap: () {
                          Navigator.push(
                              context as BuildContext,
                              PageTransition(
                                  child: MechanicPageUID(
                                      _mechanics[pinindex]["id"],
                                      _mechanics[pinindex]),
  ...
  //   try {
  //     List<Placemark> placemark = await Geolocator().placemarkFromPosition(
  //         Position(
  ...
  _LoadMarkersFromDatabase() async {
    if (_mechanics.isNotEmpty) {
      for (int i = 0; i < _mechanics.length; i++) {
        final Uint8List markerIcon =
            await getBytesFromAsset('assets/images/mechanic.png', 120);
        _markers.add(Marker(
            // This marker id can be anything that uniquely identifies each marker.
            markerId: MarkerId(_mechanics[i].id),
            position: LatLng(double.parse(_mechanics[i]['lat'].toString()),
                double.parse(_mechanics[i]['long'].toString())),
  ...
  void _launchMapsUrl() async {
    ...
    final url =
        'https://www.google.com/maps/dir/?api=1&origin=${_position.latitude},${_position.longitude}&destination=${_mechanics[pinindex]["lat"]},${_mechanics[pinindex]["long"]}&travelmode=driving';
  ...
  _getpunctured(String selection) async {
    ...
    if (_onlypuncture.isEmpty) {
      QuerySnapshot querySnapshot = await FirebaseFirestore.instance
          .collection("mechanic")
          .where("verified", isEqualTo: true)
          .where("onlypuncture", isEqualTo: true)
          .get();

      if (querySnapshot.docs.isNotEmpty) {
        _onlypuncture = querySnapshot.docs;
      }
    }
  ...
  _removefilters(List<DocumentSnapshot> _mechanics) async {
    _markers.clear();
    for (int i = 0; i < _mechanics.length; i++) {
      double lat = double.tryParse(_mechanics[i]['lat'].toString());
      double long = double.tryParse(_mechanics[i]['long'].toString());

      if (lat != null && long != null) {
        BitmapDescriptor bitmapDescriptor =
            await BitmapDescriptor.fromAssetImage(
                ImageConfiguration(), "assets/images/mechanic.png");
        _markers.add(Marker(
            // This marker id can be anything that uniquely identifies each marker.
            markerId: MarkerId(_mechanics[i].id),
  ...
  _updatemarkersonfilter(List<DocumentSnapshot> _mechanics) async {
    _markers.clear();
    for (int i = 0; i < _mechanics.length; i++) {
      ...
      _markers.add(
        Marker(
          ...
          markerId: MarkerId(_mechanics[i].id),
          position: LatLng(
            double.parse(_mechanics[i]['lat'].toString()),
            double.parse(_mechanics[i]['long'].toString()),
          ),
          icon: markerIcon,
        ),
    ...
    // Check if _position is not null before using its properties
    if (_position != null) {
      _mapcontroller.animateCamera(
        CameraUpdate.newCameraPosition(
          CameraPosition(
  ...
  _setup_filter_menu() {
    filters = [
      CustomPopupMenu(
          ...
          onclick: () {
            filtertitle = "Two wheeler puncture";
            _getpunctured("two");
          },
          title: "Two wheeler"),
      CustomPopupMenu(
          ...
          onclick: () {
            filtertitle = "Three wheeler puncture";
            _getpunctured("three");
          },
          title: "Three wheeler"),
      CustomPopupMenu(
          ...
          onclick: () {
            filtertitle = "Four wheeler puncture";
            _getpunctured("four");
          },
          title: "Four wheeler"),
      CustomPopupMenu(
          ...
          onclick: () {
            filtertitle = "Heavy vehicle puncture";
            _getpunctured("more");
          },
      ...
      children: <Widget>[
              ...
              child: InkWell(
                ...
                child: Row(
                  ...
                  children: <Widget>[
                    ...
                    Text(
                      "Search",
                      style: GoogleFonts.lato(color: LightColor.background),
                    ),
                  ],
                ),
              ),
        ...
        ListTile(
          ...
          title: Text(
            "Filters",
            style: GoogleFonts.lato(
                color: grey, fontWeight: FontWeight.w400, fontSize: 20),
          ),
        ),
        Divider(
          height: 1,
